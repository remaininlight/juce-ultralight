...
void SplitComponent::SetSplitBarGravity(double gravity) noexcept
{
...
double SplitComponent::GetSplitBarGravity() const noexcept
{
...
void SplitComponent::parentSizeChanged()
{
	...
	if (pParentComponent && m_fitToParent)
	{
		...
		Rectangle<int> bnds= pParentComponent->getBounds();
		bnds.setPosition(0, 0);
		setBounds(bnds);	// This will cause SplitComponent::resized() to be called.
	}
}
...
void SplitComponent::SplitBar::mouseDown(const juce::MouseEvent&)
{
	const Point<int> pos = getPosition();
	m_mouseDownPos = m_isVertical ? pos.getX() : pos.getY();
}

void SplitComponent::SplitBar::mouseDrag(const juce::MouseEvent& e)
{
